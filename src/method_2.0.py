    ...
    for sentence in openIEDict.keys():
        ...
        for wordPairs in openIEDict[sentence]:
            ...
            for word in annotatedDict[sentence]:
                if word == '' or '' in wordPairs:
                    continue
                if len(wordPairs[0].split(" ")) < len(word.split(" ")):
                    if checkContain(wordPairs[0],word):
                        flag1 = 1
                        word1 = wordPairs[0]
                        break
                elif len(wordPairs[0].split(" ")) >= len(word.split(" ")):
                    if checkContain(word,wordPairs[0]):
                        flag1 = 1
                        word1 = word
                        break
            flag2 = 0
            for word in annotatedDict[sentence]:            
                if len(wordPairs[2].split(" ")) < len(word.split(" ")):
                    if checkContain(wordPairs[2],word):
                        flag2 = 1
                        word2 = wordPairs[2]
                        break
                elif len(wordPairs[2].split(" ")) >= len(word.split(" ")):
                    if checkContain(word,wordPairs[2]):
                        flag2 = 1
                        word2 = word 
                        break
...
def calculateSimilarity(wordVectorDict):
    ...
    return similarityDict
def outputFilePath():
    if not os.path.exists(CURRENT_PATH+"/../output/Results_v2"):
        os.mkdir(CURRENT_PATH+"/../output/Results_v2")
    print CURRENT_PATH+"/../output/Results_v2/graph.gexf"
    return CURRENT_PATH+"/../output/Results_v2/graph.gexf"
#----Author : Ashish ends-------
